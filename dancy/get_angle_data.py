        ...
        for df in df_list:
            ...
            df3 = df2[
                [
                    ...
                    "LEFT_PINKY_y",
                    "LEFT_PINKY_z",
                    "LEFT_PINKY_visibility",
                    "RIGHT_PINKY_x",
                    "RIGHT_PINKY_y",
                    "RIGHT_PINKY_z",
                    "RIGHT_PINKY_visibility",
                    "LEFT_INDEX_x",
                    "LEFT_INDEX_y",
                    "LEFT_INDEX_z",
                    "LEFT_INDEX_visibility",
                    "RIGHT_INDEX_x",
                    "RIGHT_INDEX_y",
                    "RIGHT_INDEX_z",
                    "RIGHT_INDEX_visibility",
                    "LEFT_THUMB_x",
                    "LEFT_THUMB_y",
                    "LEFT_THUMB_z",
                    "LEFT_THUMB_visibility",
                    "RIGHT_THUMB_x",
                    "RIGHT_THUMB_y",
                    "RIGHT_THUMB_z",
                    "RIGHT_THUMB_visibility",
                    "LEFT_HIP_x",
                    "LEFT_HIP_y",
                    "LEFT_HIP_z",
                    "LEFT_HIP_visibility",
        ...
        for idx, df in enumerate(df_list):
            ...
            for j in range(df.shape[0]):
                ...
                for i in range(0, df.shape[1] - 4, 3):
                    ...
                    final_col_name = (
                        f"{before_angle_col_name}-{next_angle_col_name} angle"
                    )
            ...
            angle_df = pd.DataFrame(
                angle_array, columns=columns, index=pd.Index(list(range(df.shape[0])))
            )
            angle_df["label"] = idx
            temp_list.append(angle_df)
        result_angle_df = pd.concat(temp_list)
        result_angle_df.to_csv(
            self.save_csv_path, index=False, encoding="utf8", sep=","
        )
        print("데이터 수집 완료 !")
