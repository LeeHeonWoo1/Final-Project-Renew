<script>
    import { Sidebar, SidebarGroup, SidebarItem, SidebarWrapper, P } from 'flowbite-svelte';
...
<div id='pagination'>
    <button on:click={()=>{GetArticleList($page-1, $section)}}>&laquo;</button>
    {#each Array(total_page) as _, loop_page}
        {#if loop_page >= $page-5 && loop_page <= $page+5}
            <button class="page">{loop_page+1}</button>
        {/if}
    {/each}
    <button on:click={()=>{GetArticleList($page+1, $section)}}>&raquo;</button>
</div>
